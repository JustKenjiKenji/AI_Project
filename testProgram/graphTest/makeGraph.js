...
var gainSynth = audioContext.createGain();
gainSynth.connect(audioContext.destination);
gainSynth.gain.value=0.3;

var gainBass = audioContext.createGain();
gainBass.connect(audioContext.destination);
gainBass.gain.value=0.7;

var gainHit = audioContext.createGain();
...
function synth(pitch, duration){
    return {gain:gainSynth,preset:_tone_0480_Chaos_sf2_file,pitch:pitch,duration:duration*N};
}
	
function bass(pitch, duration){
    return {gain:gainBass,preset:_tone_0390_Aspirin_sf2_file,pitch:pitch,duration:duration*N};
}
...
function open(){return {
    gain:gainDrums,preset:_drum_46_6_JCLive_sf2_file,pitch:46,duration:1};
}
...
function extractInstrumentName(event) {
    // event is like hihat(), bass(C3,1/16), synth(C4,1/1)
    return event.name; // if your functions are named
}

function buildTracks(notes) {
    const tracks = {};

    for (let step = 0; step < notes.length; step++) {
        for (const event of notes[step]) {
            const name = extractInstrumentName(event);

            if (!tracks[name]) {
                tracks[name] = [];
            }

            tracks[name].push(step);
        }
    }
...
function drawCanvas(tracks, steps) {
    ...
    canvas.width = steps * cellWidth + 100;
    canvas.height = Object.keys(tracks).length * rowHeight + 20;

    let row = 0;

    for (const inst in tracks) {
        const y = row * rowHeight + 15;
        ctx.fillText(inst, 10, y);

        for (const step of tracks[inst]) {
            const x = 100 + step * cellWidth;
            ctx.fillRect(x, y - 10, cellWidth - 1, 12);
        }

        row++;
    }
}
const tracks = buildTracks(notes);
console.log(tracks);
drawCanvas(tracks, notes.length);
